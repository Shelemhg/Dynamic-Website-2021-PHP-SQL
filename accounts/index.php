...
switch ($action) {
    ...
    case 'Login':
        ...
        $storedPassword = $clientData['clientPassword'];
        // Compare the password just submitted against
        // the hashed password for the matching client
        $hashCheck = password_verify($clientPassword, $storedPassword);
    ...
    case 'updateClient':
        ...
        if($clientEmail != $clientInfo['clientEmail']){
            // Check if email already exists
            $existingEmail = checkExistingEmail($clientEmail);
            if($existingEmail){
                $message = '<p class="notice">That email address already exists. Do you want to login instead?</p>';
    ...
    case 'updatePassword':
        ...
        if(empty($checkPassword)) {
            $message = '<p>ERROR - Please enter a password that matches the corresponding format.</p>';
            include $_SERVER['DOCUMENT_ROOT'] .'/phpmotors/view/client-update.php';
            exit; 
        }
        // Hash the client password if the above if didn't exit the program
        $hashedPassword = password_hash($clientPassword, PASSWORD_DEFAULT);

        $updatedPassword = updatePassword($hashedPassword, $clientId);
        // Check and report the result
        if($updatedPassword){
            // setcookie('firstname', $clientFirstname, strtotime('+1 year'), '/');
